<template>
    <div class="algorithm" style="margin-top:60px;">
            
                <h1 class="title" style="margin-top:0;margin-left:30px">创建训练任务</h1>
                <div id="form">
    ...
    export default {
        ...
        data() {
            return {
                ...
                search: "",
                algorithmsList: [{"id":"1","name":"FetureOutlier","index":"1"},{"id":"2","name":"SequenceOutlier","index":"2"}],
                dataList: [{"id":"1","name":"train.csv"},{"id":"2","name":"test.txt"}],
            ...
            onSubmit(formName) {
                this.$refs[formName].validate(valid => {
                    if (valid) {
                        ...
                        let algo = "";
                        console.log(formToPost.algoId)
                        if(formToPost.algoId==1){
                            algo = "FeatureOutlier";

                        }else{
                            algo = "SequenceOutlier";
                        }
                        console.log(new Date().getTime());
                        let data1="";
                        if(formToPost.dataId==1){
                            data1="train.csv";
                        }else{
                            data1="test.txt";
                        }
                        let task = {id:2, "name":formToPost.name,"algo":algo,"algoType":"supervised","data":data1,startTime: new Date().getTime(),"status":2}
                        this.$axios
            ...
            getTaskList() {
                ...
                this.taskList = this.data.map((element, index) => {
                    ...
                    if(element.endTime!==null){
                        let duration = (element.endTime - element.startTime)/1000*1000;
